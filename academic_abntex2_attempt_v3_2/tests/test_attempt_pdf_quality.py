...
class TestPDFQuality:
    ...
    # --- v3.2: Operational privacy checks ---
    def test_pdf_contains_pseudonimizacao(self, pdf_text):
        lower = pdf_text.lower()
        assert "pseudonimiza" in lower, "PDF nao contem pseudonimizacao"

    def test_pdf_contains_reidentificacao_restrita(self, pdf_text):
        lower = pdf_text.lower()
        assert "reidentifica" in lower, "PDF nao contem reidentificacao"

    def test_pdf_contains_case_id(self, pdf_text):
        lower = pdf_text.lower()
        assert ("case" in lower and
                ("case-2026" in lower or "identiﬁcador de caso" in lower or
                 "identificador de caso" in lower or "case_id" in lower or
                 "case-" in lower))

    def test_pdf_contains_equipe_autorizada(self, pdf_text):
        lower = pdf_text.lower()
        assert "equipe" in lower and "autorizada" in lower

    def test_pdf_contains_chave_reidentificacao(self, pdf_text):
        lower = pdf_text.lower()
        assert "chave" in lower and "reidentifica" in lower

    def test_pdf_contains_painel_analitico(self, pdf_text):
        lower = pdf_text.lower()
        assert "painel" in lower and "anal" in lower

    def test_pdf_contains_supressao_grupos(self, pdf_text):
        lower = pdf_text.lower()
        assert "supress" in lower and "grupo" in lower

    def test_pdf_contains_pse_ubs_caps(self, pdf_text):
        assert "PSE" in pdf_text
        assert "UBS" in pdf_text
        assert "CAPS" in pdf_text

    def test_pdf_no_gestao_inteira(self, pdf_text):
        lower = pdf_text.lower()
        assert "gestao inteira" not in lower
        assert "gest~ao inteira" not in pdf_text

    def test_pdf_no_ranking_de_alunos_permitido(self, pdf_text):
        lower = pdf_text.lower()
