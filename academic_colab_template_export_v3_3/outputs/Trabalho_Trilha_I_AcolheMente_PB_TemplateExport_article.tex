...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Figura --- Fluxo de proveniência de dados: cada tier alimenta exclusivamente seu destino legítimo.}]{fluxo_proveniencia_dados.png}}
\caption{Figura --- Fluxo de proveniência de dados: cada tier alimenta
exclusivamente seu destino legítimo.}
\end{figure}
...
\begin{quote}
O código-fonte completo da definição formal dos símbolos, incluindo
tipos, descrições e rastreabilidade PeNSE, está disponível no Apêndice
A.
\end{quote}
...
\begin{quote}
A versão de produção inclui também o \textbf{grafo de explicabilidade}
(Apêndice E), que visualiza as relações regra-variável via NetworkX, e o
\textbf{gerador de tabelas de resultados} (Apêndice F), que produz
tabelas LaTeX programáticas dos cenários de validação.
\end{quote}

\begin{quote}
O código-fonte completo do projeto, incluindo ambas as versões do motor,
testes automatizados, documentação de decisões arquiteturais (ADRs) e
scripts de validação, está disponível no repositório público:
\href{https://github.com/lmbernardo7520112/Atividade_Final_Trilha_I_Ifgoiano}{github.com/lmbernardo7520112/Atividade\_Final\_Trilha\_I\_Ifgoiano}.
\end{quote}

\subsubsection{Engenharia de Software e Garantia de
Qualidade}\label{engenharia-de-software-e-garantia-de-qualidade}

\begin{quote}
O desenvolvimento seguiu práticas profissionais de engenharia de
software: - \textbf{Specification Driven Development (SDD):} Cada
funcionalidade foi especificada formalmente antes da implementação
(cf.~\texttt{docs/specs/}). - \textbf{Test Driven Development (TDD):} 8
suítes de testes de contrato (\texttt{tests/test\_*.py}) verificam
invariantes como equivalência dos motores, rastreabilidade PeNSE,
guardrails de não diagnóstico e integridade acadêmica. - \textbf{Clean
Code:} Código modular com separação de responsabilidades, docstrings em
português, nomes descritivos e menos de 100 linhas por módulo. -
\textbf{Conventional Commits:} Histórico de commits rastreável com
prefixos semânticos (\texttt{fix:}, \texttt{docs:}, \texttt{feat:}). -
\textbf{Architectural Decision Records (ADRs):} Decisões críticas
documentadas formalmente (cf.~ADR sobre R5 e subsunção). -
\textbf{Cobertura exaustiva:} Validação com \(2^6 = 64\) combinações,
100\% determinística, sem dependência de dados externos. -
\textbf{Engenharia de Software Assistida por IA:} O ciclo de
desenvolvimento, refatoração de código e criação de pipelines
automatizados contou com o suporte instrumental de inteligência
artificial generativa (Antigravity), atuando estritamente como agente
assistivo sob direcionamento arquitetural do autor humano (cf.~Apêndice
G).
\end{quote}
...
\begin{quote}
As tabelas de variáveis, regras e cenários apresentadas neste trabalho
podem ser reproduzidas programaticamente pelo script
\texttt{gerar\_tabelas\_resultados.py}, disponível no Apêndice F. O
gerador produz as mesmas tabelas em formato LaTeX, garantindo
consistência entre o texto e os dados validados.
\end{quote}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Figura --- Ativação de regras por cenários sintéticos representativos.}]{cenarios_ativacao_regras.png}}
\caption{Figura --- Ativação de regras por cenários sintéticos
representativos.}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Figura --- Comparação qualitativa: baseline manual vs.~motor lógico.}]{comparacao_baseline_motor.png}}
\caption{Figura --- Comparação qualitativa: baseline manual vs.~motor
lógico.}
\end{figure}
...
\begin{quote}
Para materializar essa rastreabilidade, o Apêndice E apresenta o
código-fonte do \textbf{grafo de explicabilidade}
(\texttt{grafo\_explicabilidade\_acolhemente.py}). O grafo, construído
com NetworkX e renderizado com Matplotlib, representa visualmente as
relações entre variáveis de entrada e a variável de saída \(A\),
mediadas pelas regras R1--R6. Cada aresta corresponde a uma regra
formal, permitindo que qualquer conclusão do motor seja verificada
visualmente. A geração é 100\% determinística, sem dependência de LLMs
ou APIs externas.
\end{quote}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Figura --- Grafo de regras: 6 variáveis de entrada, 6 regras R1--R6 e 1 saída inferida (A).}]{grafo_regras_7vars.png}}
\caption{Figura --- Grafo de regras: 6 variáveis de entrada, 6 regras
R1--R6 e 1 saída inferida (A).}
\end{figure}
...
\begin{quote}
Está prevista a adição de pipeline CI/CD via GitHub Actions para
execução automatizada dos testes de contrato a cada atualização do
repositório.
\end{quote}
...
\section*{Apêndice G --- Declaração de Uso de Inteligência Artificial}
\addcontentsline{toc}{section}{Apêndice G --- Declaração de Uso de Inteligência Artificial}

Este trabalho acadêmico contou com o uso de ferramentas de Inteligência Artificial generativa (\textit{Antigravity/Modelos de Linguagem Avançados}) exclusivamente como apoio instrumental, em conformidade com as diretrizes da CAPES (Brasil) para transparência no uso de IA em produções acadêmicas. A utilização deu-se em dois eixos:

\begin{enumerate}
  \item \textbf{Engenharia de Software Assistida:} apoio na automação de pipelines, refatoração de código limpo (\textit{clean code}), geração de \textit{boilerplates} para testes de contrato (TDD) e formatação de scripts \LaTeX.
  \item \textbf{Revisão Formal:} apoio à revisão ortográfica, à estruturação de tópicos e à adequação do texto às normas da ABNT.
\end{enumerate}

Ressalta-se que a Inteligência Artificial \textbf{não foi utilizada como autora do trabalho}, nem como substituta da elaboração intelectual do discente. Todas as escolhas metodológicas, desenho arquitetural, engenharia de \textit{guardrails}, concepção das regras de negócio (R1--R6), interpretações teóricas e conclusões acadêmicas são de inteira responsabilidade do autor humano. A formulação do modelo lógico proposicional e as salvaguardas de conformidade com a LGPD e o ECA são concepções exclusivamente humanas.

O conteúdo submetido ao apoio da ferramenta passou por revisão crítica, conferência minuciosa (via \textit{quality gates} e testes de contrato) e validação humana, com o objetivo de assegurar a correção das informações, a coerência argumentativa, a conformidade com as regras da ABNT e a adequação acadêmica do texto final.

Assim, o uso da IA neste trabalho teve caráter estritamente instrumental, limitado ao suporte arquitetural de \textit{software}, linguístico, estrutural e normativo, \textbf{sem transferência de autoria ou de responsabilidade acadêmica}.
