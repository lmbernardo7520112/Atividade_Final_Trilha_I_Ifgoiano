...
\subsection{Apêndice A: Símbolos Proposicionais do
AcolheMente}\label{apuxeandice-a-suxedmbolos-proposicionais-do-acolhemente}

\label{apendice:simbolos} \textgreater{} Código-fonte completo no
arquivo \texttt{simbolos\_acolhemente.py} (salvo via
\texttt{\%\%writefile} acima).

\subsection{Apêndice B: Regras Lógicas e
CNF}\label{apuxeandice-b-regras-luxf3gicas-e-cnf}

\label{apendice:regras} \textgreater{} Código-fonte completo no arquivo
\texttt{regras\_acolhemente.py} (salvo via \texttt{\%\%writefile}
acima).

\subsection{Apêndice C: Motor de Inferência por
Resolução}\label{apuxeandice-c-motor-de-inferuxeancia-por-resoluuxe7uxe3o}

\label{apendice:motor} \textgreater{} Código-fonte completo no arquivo
\texttt{motor\_resolucao\_acolhemente.py} (salvo via
\texttt{\%\%writefile} acima).

\subsection{Apêndice D: Cenários Sintéticos de
Validação}\label{apuxeandice-d-cenuxe1rios-sintuxe9ticos-de-validauxe7uxe3o}

\label{apendice:cenarios} \textgreater{} Código-fonte completo no
arquivo \texttt{cenarios\_sinteticos\_acolhemente.py} (salvo via
\texttt{\%\%writefile} acima).

\subsection{Apêndice E: Grafo de
Explicabilidade}\label{apuxeandice-e-grafo-de-explicabilidade}

\label{apendice:grafo} \textgreater{} Código-fonte completo no arquivo
\texttt{grafo\_explicabilidade\_acolhemente.py} (salvo via
\texttt{\%\%writefile} acima).

\subsection{Apêndice F: Gerador de Tabelas de
Resultados}\label{apuxeandice-f-gerador-de-tabelas-de-resultados}

\label{apendice:tabelas} \textgreater{} Código-fonte completo no arquivo
\texttt{gerar\_tabelas\_resultados.py} (salvo via \texttt{\%\%writefile}
acima).
